// 🚀 Version Deno de votre script extract-oic-competences.cjs
...
const config = {
  ...
  supabase: {
    ...
    serviceKey: Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlhaW5jb3hpaGlxZGtzeGdyc3JrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0MjgxMTgyNywiZXhwIjoyMDU4Mzg3ODI3fQ.w0j3LPGvYEhKz_eO6rYjYEo8VNjzGjLOGd8YCLBqNrE'
  },
  urls: {
...
async function extractAllCompetences() {
  ...
  try {
    // 1. Authentification CAS (version Deno, sans Puppeteer)
    ...
    const rapport = {
      ...
      method: 'Deno + CAS Authentication',
    ...
    const reportContent = JSON.stringify(rapport, null, 2)
    const reportFile = `rapport-deno-${new Date().toISOString().slice(0,10)}.json`
...
async function authenticateCASWithDeno(): Promise<string | null> {
  ...
  try {
    ...
    // Étape 3: Soumettre les identifiants (email + mot de passe en une fois)
...
async function extractViaAPIWithDeno(cookies: string, stats: any): Promise<any[]> {
  ...
  do {
    ...
    // Filtrer les pages OIC selon votre pattern
...
// Exécuter l'extraction si ce script est lancé directement
if (import.meta.main) {
  log('🚀 Lancement de l\'extraction OIC avec Deno...')
  
  extractAllCompetences()
    .then(rapport => {
      log('🎉 Script Deno terminé avec succès !')
      log(`📊 Rapport final: ${JSON.stringify(rapport, null, 2)}`)
    })
    .catch(error => {
      log(`❌ Erreur critique dans le script Deno: ${error.message}`)
      Deno.exit(1)
    })
}