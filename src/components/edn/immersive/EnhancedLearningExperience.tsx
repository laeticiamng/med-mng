...
import { Settings, BarChart3, Route, Lightbulb, Volume2, VolumeX, Maximize, Minimize, Palette, Moon, Sun, Focus, Play, Pause } from 'lucide-react';
...
export const EnhancedLearningExperience: React.FC<EnhancedLearningExperienceProps> = ({
  ...
  const getCurrentPerformance = () => {
    const avgScore = Object.values(userProgress.performanceScores).reduce((a, b) => a + b, 0) / Object.values(userProgress.performanceScores).length || 75;
  ...
  const SettingsPanel = () => <Card className="w-80">
      <CardContent className="p-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => updateSetting('soundEnabled', !settings.soundEnabled)}>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => updateSetting('immersiveMode', !settings.immersiveMode)}>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => updateSetting('darkMode', !settings.darkMode)}>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => updateSetting('focusMode', !settings.focusMode)}>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {(['minimal', 'normal', 'verbose'] as const).map(level => <Button key={level} size="sm" variant={settings.hintLevel === level ? 'default' : 'outline'} onClick={() => updateSetting('hintLevel', level)} className="text-xs">
                  {level}
                </Button>)}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {(['slow', 'normal', 'fast'] as const).map(speed => <Button key={speed} size="sm" variant={settings.animationSpeed === speed ? 'default' : 'outline'} onClick={() => updateSetting('animationSpeed', speed)} className="text-xs">
                  {speed}
                </Button>)}
            </div>
          </div>
  ...
  return <div className={`relative ${settings.focusMode ? 'focus-mode' : ''}`}>
      ...
      {/* Panels flottants */}
      <AnimatePresence>
        {activePanel === 'analytics' && <motion.div initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
        ...
        y: -20
            ...
            <ProgressAnalytics itemCode={itemCode} currentSection={currentSection} completedSections={userProgress.completedSections} />
          </motion.div>}

        {activePanel === 'paths' && <motion.div initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
        ...
        y: -20
            ...
            <LearningPathSuggestions itemCode={itemCode} currentSection={currentSection} userProgress={userProgress} onPathSelect={handlePathSelect} onSectionNavigate={onSectionChange} />
          </motion.div>}

        {activePanel === 'settings' && <motion.div initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
      ...
      {/* Hints intelligents */}
      {settings.hintLevel !== 'minimal' && <SmartHints currentSection={currentSection} timeSpentInSection={(Date.now() - currentSectionStartTime) / 1000} interactionCount={interactionCount} performanceScore={getCurrentPerformance()} itemCode={itemCode} />}

      {/* Contenu principal avec wrapper d'interaction */}
      <div onClick={handleInteraction} onKeyDown={handleInteraction} className="min-h-screen">
        {children}
      </div>

      {/* Overlay pour mode focus */}
      <AnimatePresence>
        {settings.focusMode && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
