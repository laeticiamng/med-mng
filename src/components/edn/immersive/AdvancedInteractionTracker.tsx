...
export const AdvancedInteractionTracker: React.FC<AdvancedInteractionTrackerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = (e: Event) => {
      ...
      if (scrollEvents.length > 5) {
        ...
        const speeds = recentScrolls.slice(1).map((scroll, i) => Math.abs(scroll.y - recentScrolls[i].y) / (scroll.time - recentScrolls[i].time));
        const avgSpeed = speeds.reduce((a, b) => a + b, 0) / speeds.length;
        if (avgSpeed > 2) setScrollBehavior('rapid');else if (avgSpeed < 0.5) setScrollBehavior('smooth');else setScrollBehavior('erratic');
      }
      setInteractionData(prev => ({
        ...prev,
  ...
  const calculateMovementComplexity = (movements: {
    x: number;
    y: number;
    time: number;
  }[]) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setInteractionData(prev => {
        ...
        const engagementScore = newData.clicks * 2 + newData.scrolls * 1 + newData.keyPresses * 3 + newData.focusTime * 0.1 - newData.idleTime * 0.2;
        ...
        if (focusRatio > 0.9 && activityScore > 20) newData.engagementLevel = 'peak';else if (focusRatio > 0.7 && activityScore > 10) newData.engagementLevel = 'high';else if (focusRatio > 0.5 && activityScore > 5) newData.engagementLevel = 'medium';else newData.engagementLevel = 'low';
        return newData;
      });
    }, 1000);
  ...
  return <div className="relative">
      ...
      {process.env.NODE_ENV === 'development' && <div className="fixed pointer-events-none z-50 w-2 h-2 bg-red-500 rounded-full opacity-50" style={{
