      ...
      <MedMngLayout>
        <ConsistentBackground variant="primary">
          <div className={`container mx-auto px-4 py-8 ${spacing.container}`}>
            <div className="mb-8">
              <h1 className="text-2xl sm:text-4xl font-bold text-white mb-2" id="main-content">
                Ma bibliothèque musicale
              </h1>
              <p className="text-white/80">Chargement de vos chansons...</p>
            </div>
            
            <SkeletonLibraryGrid count={12} />
            
            {showSlowLoading && (
              <div className="text-center mt-8 p-4 bg-white/10 rounded-lg backdrop-blur">
        ...
        <div className={`container mx-auto px-4 py-8 ${spacing.container}`}>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="text-right">
              <div className="bg-white/10 backdrop-blur rounded-lg px-3 py-2 shadow-sm">
                <TranslatedText text="Crédits restants" className="text-xs sm:text-sm text-white/80" />
          ...
          <div className={`flex ${gridConfig.navigation} ${gridConfig.gap} mb-6 md:mb-8`}>
            <Button 
              onClick={() => navigate('/med-mng/create')}
              className={`flex items-center gap-2 bg-white text-primary hover:bg-white/90 min-h-[48px] ${isMobile ? 'w-full' : 'w-auto'}`}
            ...
            <Button 
              ...
              className={`min-h-[48px] border-white/20 text-white hover:bg-white/10 ${isMobile ? 'w-full' : 'w-auto hidden sm:flex'}`}
          ...
          {filteredSongs.length === 0 ? (
            <div className="text-center py-16">
              ...
              {(!library || library.length === 0) && (
                <Button 
                  onClick={() => navigate('/med-mng/create')} 
                  className={`bg-white text-primary hover:bg-white/90 min-h-[48px] px-6 ${isMobile ? 'w-full' : ''}`}
            ...
            <div className="flex justify-center mt-6 md:mt-8">
              <div className={`flex ${isMobile ? 'flex-col w-full' : 'gap-2'} ${gridConfig.gap}`}>
                <Button
                  ...
                  className={`min-h-[44px] px-4 sm:px-6 border-white/20 text-white hover:bg-white/10 ${isMobile ? 'w-full mb-2' : 'flex-1 sm:flex-none'}`}
                ...
                <Button
                  ...
                  className={`min-h-[44px] px-4 sm:px-6 border-white/20 text-white hover:bg-white/10 ${isMobile ? 'w-full' : 'flex-1 sm:flex-none'}`}
