-- Seed baseline integrity checks and a reference snapshot for development
...
values
    ...
    ('db_constraints_valid', 'database', 'high', 'Critical indexes and constraints are present for library, analytics and lyrics tables.', 'Re-run the latest SQL hardening migration or execute `pnpm integrity:audit` to inspect missing assets.'),
...
insert into public.deployment_integrity_snapshots (environment, check_name, status, details, executed_at)
values
    ('development', 'database_migrations_applied', 'pass', '{"notes":"local bootstrap"}'::jsonb, '2024-01-01T00:00:00+00'),
    ('development', 'rls_policies_enforced', 'pass', '{"notes":"local bootstrap"}'::jsonb, '2024-01-01T00:00:00+00'),
    ('development', 'seed_data_loaded', 'warn', '{"notes":"development seeds intentionally partial"}'::jsonb, '2024-01-01T00:00:00+00');

on conflict (environment, check_name, executed_at) do nothing;
