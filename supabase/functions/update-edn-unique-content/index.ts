...
async function generateAdvancedLyricsForAllItems(supabase: any) {
  console.log('🚀 Génération des paroles médicales avancées pour tous les items EDN...');
  ...
  try {
    ...
    // Traitement par batch de 3 pour éviter la surcharge OpenAI
    const batchSize = 3;
    
    for (let i = 0; i < items.length; i += batchSize) {
      const batch = items.slice(i, i + batchSize);
      console.log(`🔄 Traitement batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(items.length/batchSize)}`);
      ...
      for (const item of batch) {
        try {
          ...
          // Générer les 3 versions avec OpenAI
          const [lyricsA, lyricsB, lyricsAB] = await Promise.all([
            generateSpecificLyrics(item.item_code, item.title, 'A'),
            generateSpecificLyrics(item.item_code, item.title, 'B'), 
            generateSpecificLyrics(item.item_code, item.title, 'AB')
          ]);
          ...
          result.successful++;
          console.log(`✅ ${item.item_code} - Paroles médicales spécifiques générées`);
      ...
      // Pause entre les batches pour respecter les limites OpenAI
      if (i + batchSize < items.length) {
        await new Promise(resolve => setTimeout(resolve, 3000));
      }
    }
    
    console.log('🎉 Génération médicale terminée:', result);
    return new Response(
...
async function generateSpecificLyrics(itemCode: string, title: string, rang: 'A' | 'B' | 'AB'): Promise<string[]> {
  if (!openAIApiKey) {
    console.log('⚠️ Clé OpenAI manquante, utilisation du contenu médical structuré');
    return generateMedicalContentLyrics(itemCode, title, rang);
  }

  // Créer un prompt médical spécifique selon le rang et l'item
  const prompt = createMedicalLyricsPrompt(itemCode, title, rang);
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: `Tu es un expert médical français qui crée des paroles de chansons éducatives très précises pour les étudiants en médecine. 
            Tu dois créer des paroles avec les VRAIES notions médicales détaillées qui permettront aux étudiants d'avoir 20/20 à l'examen EDN.
            Les paroles doivent être riches en contenu médical CONCRET, PRÉCIS et FACTUEL.
            Style: accessible mais très informatif, avec des termes médicaux précis et actuels.
            Format: 15-25 lignes maximum, chaque ligne doit apporter une information médicale utile et précise.
            Utilise des rimes simples et mémorisables.`
          },
          { role: 'user', content: prompt }
        ],
        temperature: 0.2, // Très peu de créativité pour maximiser la précision médicale
        max_tokens: 1000
      }),
    });
    ...
    console.error(`Erreur génération OpenAI ${itemCode} ${rang}:`, error);
    // Fallback avec contenu médical structuré
    return generateMedicalContentLyrics(itemCode, title, rang);
  }
}

function createMedicalLyricsPrompt(itemCode: string, title: string, rang: 'A' | 'B' | 'AB'): string {
  const itemNumber = parseInt(itemCode.replace('IC-', ''));
  const domain = getDomainForItem(itemNumber);
  const specializedData = SPECIALIZED_CONTENT[domain] || SPECIALIZED_CONTENT["fondamentaux"];
  
  const basePrompt = `Crée des paroles de chanson médicale éducative très spécifiques pour l'item EDN ${itemCode}: "${title}"
  
Spécialité médicale: ${specializedData.specialty}
Contexte clinique: ${specializedData.context}`;
  
  switch (rang) {
    case 'A':
      return `${basePrompt}
      
RANG A (Connaissances de base essentielles) - Inclus OBLIGATOIREMENT ces éléments médicaux précis:
- Définitions médicales exactes et terminologie clinique
- Physiopathologie de base avec mécanismes précis
- Signes cliniques spécifiques et symptomatologie
- Examens de première intention et leurs indications
- Épidémiologie chiffrée et facteurs de risque
- Éléments diagnostiques différentiels principaux

CONTENU MÉDICAL SPÉCIALISÉ À INTÉGRER:
${specializedData.rangA.map(item => `- ${item}`).join('\n')}

MÉTHODES DIAGNOSTIQUES:
${specializedData.diagnosticMethods.map(item => `- ${item}`).join('\n')}

Format: 15-20 lignes de chanson avec rimes
Chaque ligne DOIT contenir une information médicale concrète, factuelle et précise
Exemple: "Infarctus STEMI avec sus-ST en antérieur, troponine élevée signe la douleur"
Ne pas utiliser de termes vagues, tout doit être spécifique à l'item ${itemCode}`;

    case 'B':
      return `${basePrompt}
      
RANG B (Connaissances approfondies expertes) - Inclus OBLIGATOIREMENT:
- Mécanismes physiopathologiques complexes et moléculaires
- Examens complémentaires spécialisés et leurs interprétations
- Stratégies thérapeutiques avancées et protocoles
- Complications spécifiques et leur prise en charge
- Pronostic précis et facteurs pronostiques
- Particularités selon terrain, âge et comorbidités

EXPERTISE AVANCÉE À INTÉGRER:
${specializedData.rangB.map(item => `- ${item}`).join('\n')}

APPROCHES THÉRAPEUTIQUES:
${specializedData.therapeuticApproaches.map(item => `- ${item}`).join('\n')}

COMPLICATIONS SPÉCIFIQUES:
${specializedData.complications.map(item => `- ${item}`).join('\n')}

Format: 15-20 lignes de chanson avec rimes
Chaque ligne DOIT être une information médicale avancée, précise et experte
Exemple: "Thrombolyse IV dans les 4h30, rTPA dose 0,9mg/kg calculée"
Niveau expert requis pour l'item ${itemCode}`;

    case 'AB':
      return `${basePrompt}
      
RANG AB (Synthèse complète A+B - Maîtrise totale) - Inclus OBLIGATOIREMENT:
- Synthèse intégrée des connaissances de base ET approfondies
- Démarche diagnostique complète de A à Z
- Prise en charge thérapeutique globale et personnalisée
- Vision d'ensemble de la pathologie dans son contexte
- Liens avec autres pathologies et interactions
- Points clés absolument essentiels pour réussir l'examen

SYNTHÈSE COMPLÈTE:
Rang A: ${specializedData.rangA.slice(0, 3).join(', ')}
Rang B: ${specializedData.rangB.slice(0, 3).join(', ')}

SCÉNARIOS CLINIQUES TYPES:
${specializedData.clinicalScenarios.map(item => `- ${item}`).join('\n')}

Format: 20-25 lignes de chanson avec rimes
Chaque ligne DOIT apporter une information médicale essentielle pour la réussite
Exemple: "De l'anamnèse au diagnostic, puis traitement spécifique, suivi systématique"
Couvre TOUT ce qu'il faut savoir sur l'item ${itemCode} pour avoir 20/20`;
  }
}

function generateMedicalContentLyrics(itemCode: string, title: string, rang: 'A' | 'B' | 'AB'): string[] {
  const itemNumber = parseInt(itemCode.replace('IC-', ''));
  const domain = getDomainForItem(itemNumber);
  const specializedData = SPECIALIZED_CONTENT[domain] || SPECIALIZED_CONTENT["fondamentaux"];
  
  const rangInfo = rang === 'A' ? 'bases médicales' : rang === 'B' ? 'expertise avancée' : 'maîtrise complète';
  const competences = rang === 'A' ? specializedData.rangA : rang === 'B' ? specializedData.rangB : [...specializedData.rangA.slice(0, 2), ...specializedData.rangB.slice(0, 2)];
  
  return [
    `[${itemCode} - ${specializedData.specialty}]`,
    `${title.substring(0, 50)}`,
    `Spécialité: ${specializedData.specialty}`,
    `Contexte: ${specializedData.context}`,
    '',
    `[Compétences ${rang} - ${rangInfo}]`,
    ...competences.slice(0, 3).map(comp => comp.substring(0, 60)),
    '',
    `[Méthodes diagnostiques]`,
    ...specializedData.diagnosticMethods.slice(0, 2).map(method => method.substring(0, 50)),
    '',
    `[Approches thérapeutiques]`,
    ...specializedData.therapeuticApproaches.slice(0, 2).map(therapy => therapy.substring(0, 50)),
    '',
    `[${itemCode} maîtrisé]`,
    `${specializedData.specialty} expertise validée`,
    `Compétences ${rang} intégrées`,
    `Succès EDN garanti`
  ];
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'generate_advanced_lyrics') {
      return await generateAdvancedLyricsForAllItems(supabase);
    }
