...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Récupérer tous les items sans contenu structuré
    const { data: items, error: itemsError } = await supabase
      .from('edn_items_immersive')
      .select('id, item_code, title, subtitle, tableau_rang_a, tableau_rang_b')
      .is('tableau_rang_a->objectifs', null);

    if (itemsError) throw itemsError;

    console.log(`📊 ${items?.length || 0} items sans contenu trouvés`);
    ...
    // 3. Indexer les compétences par item_parent et rang
