...
-- Créer les politiques RLS (lecture publique pour les compétences, gestion admin pour la progression)
CREATE POLICY IF NOT EXISTS "Allow public read access to oic_competences" 
ON public.oic_competences FOR SELECT 
USING (true);

CREATE POLICY IF NOT EXISTS "Allow service role to manage oic_competences" 
ON public.oic_competences FOR ALL 
USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access to oic_extraction_progress" 
ON public.oic_extraction_progress FOR SELECT 
USING (true);

CREATE POLICY IF NOT EXISTS "Allow service role to manage oic_extraction_progress" 
