...
BEGIN
  ...
  FOR item_record IN 
    SELECT item_code, tableau_rang_a
    FROM edn_items_immersive
    WHERE tableau_rang_a::text LIKE '%Connaissances de base%'
       OR tableau_rang_a::text LIKE '%Concept fondamental%'
       OR tableau_rang_a::text LIKE '%Item médical%'
  LOOP
    total_issues := total_issues + 1;
    RETURN QUERY SELECT 
