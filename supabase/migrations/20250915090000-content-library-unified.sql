...
WITH track_metadata AS (
  SELECT
    s.id,
    COUNT(ls.id) AS lyric_segments
  FROM public.med_mng_songs s
  LEFT JOIN public.lyrics_segments ls ON ls.track_id = s.id
  GROUP BY s.id
)
SELECT
  'track'::text AS resource_type,
  s.id::text AS resource_identifier,
  s.user_id AS owner_id,
  false AS is_public,
  'med_mng_songs'::text AS source_table,
  s.title,
  s.description,
  array_remove(array_cat(COALESCE(s.tags, ARRAY[]::text[]), ARRAY[s.genre, s.mood, (s.meta->>'medical_focus')]), NULL) AS tags,
  jsonb_build_object(
    'duration_seconds', s.duration_seconds,
    'bpm', s.bpm,
    'has_lyrics', tm.lyric_segments > 0,
    'lyric_segments', tm.lyric_segments,
    'audio_status', s.status
  ) AS metadata,
  s.created_at,
  s.updated_at
FROM public.med_mng_songs s
LEFT JOIN track_metadata tm ON tm.id = s.id
UNION ALL
SELECT
  ...
  array_remove(array_cat(COALESCE(e.tags_medicaux, ARRAY[]::text[]), ARRAY[e.slug, e.specialite, e.domaine_medical]), NULL) AS tags,
  jsonb_build_object(
...
SELECT
  ...
  ARRAY[qs.type, qs.item_id]::text[] AS tags,
  jsonb_build_object(
