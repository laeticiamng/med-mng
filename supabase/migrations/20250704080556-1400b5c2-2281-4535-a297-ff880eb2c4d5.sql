-- Création d'une fonction pour migrer et intégrer tous les items EDN
  ...
  LOOP
    BEGIN
      ...
      tableau_rang_a_data := jsonb_build_object(
        ...
        'sections', CASE 
          WHEN item_record.rangs_a IS NOT NULL AND array_length(item_record.rangs_a, 1) > 0 THEN
            (SELECT jsonb_agg(
              jsonb_build_object(
                'title', 'Connaissance ' || (row_number() OVER()),
                'content', rang,
                'keywords', string_to_array(lower(regexp_replace(rang, '[^a-zA-Z0-9\s]', '', 'g')), ' ')
              )
            ) FROM unnest(item_record.rangs_a) AS rang)
          ELSE
            '[{"title": "Connaissances de base", "content": "Connaissances fondamentales à acquérir", "keywords": ["fondamental", "base", "connaissance"]}]'::jsonb
        END
      );
      ...
      tableau_rang_b_data := jsonb_build_object(
        ...
        'sections', CASE 
          WHEN item_record.rangs_b IS NOT NULL AND array_length(item_record.rangs_b, 1) > 0 THEN
            (SELECT jsonb_agg(
              jsonb_build_object(
                'title', 'Connaissance approfondie ' || (row_number() OVER()),
                'content', rang,
                'keywords', string_to_array(lower(regexp_replace(rang, '[^a-zA-Z0-9\s]', '', 'g')), ' ')
              )
            ) FROM unnest(item_record.rangs_b) AS rang)
          ELSE
            '[{"title": "Connaissances approfondies", "content": "Connaissances avancées et spécialisées", "keywords": ["approfondi", "avancé", "spécialisé"]}]'::jsonb
        END
      );
