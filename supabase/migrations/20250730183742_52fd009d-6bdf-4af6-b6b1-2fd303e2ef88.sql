-- Correction de la fonction d'enrichissement des items EDN
CREATE OR REPLACE FUNCTION public.enrich_edn_items_with_oic_competences_fixed()
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Récupérer les compétences Rang A pour cet item (sans ORDER BY dans l'agrégation)
      WITH rang_a_data AS (
        SELECT 
          objectif_id,
          intitule,
...
-- Exécuter la fonction d'enrichissement corrigée
SELECT * FROM public.enrich_edn_items_with_oic_competences_fixed();