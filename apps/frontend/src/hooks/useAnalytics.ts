...
export const useAnalytics = () => {
  ...
  const trackListening = async (songId: string, duration: number = 0) => {
    ...
    try {
      ...
      logger.error('Erreur tracking écoute:', error);
    }
  };
  ...
  const logEvent = async (
    ...
    try {
      ...
      logger.error('Erreur log event:', error);
    }
  };
  ...
  const toggleFavorite = async (songId: string) => {
    ...
    try {
      ...
      logger.error('Erreur toggle favorite:', error);
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const trackPerformance = async (metric: string, value: number, category: string = 'performance') => {
    ...
    try {
      ...
      logger.error('Erreur tracking performance:', error);
    }
  };
