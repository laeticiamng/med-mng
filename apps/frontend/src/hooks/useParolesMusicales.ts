...
export const useParolesMusicales = (
  ...
  const handleGenerate = async (rang: 'A' | 'B') => {
    logger.debug(`🎵 GÉNÉRATION COMPLÈTE - Rang ${rang}`);
    
    if (!itemData?.item_code) {
      logger.error('❌ CODE ITEM MANQUANT');
      toast({
    ...
    try {
      ...
      logger.debug(`🎯 Génération complète pour ${itemData.item_code} Rang ${rang}`);
      ...
      logger.debug(`✅ ${parolesCompletes.length} lignes générées avec assonances:`, {
      ...
      logger.debug(`🎵 Configuration génération:`, {
        selectedStyle,
        musicDuration,
      ...
      logger.debug(`✅ GÉNÉRATION DÉMARRÉE - ${itemData.item_code} Rang ${rang}, trackId:`, trackId);
      ...
      logger.debug('🎵 POLLING POUR RÉCUPÉRER L\'URL AUDIO...');
      ...
      logger.error(`❌ ERREUR GÉNÉRATION COMPLÈTE ${rang}:`, error);
      toast({
  ...
  const handleGenerateMix = async () => {
    logger.debug('🎵 GÉNÉRATION MIX A+B COMPLÈTE');
    
    if (!itemData?.item_code) {
      logger.error('❌ CODE ITEM MANQUANT POUR MIX');
      toast({
    ...
    try {
      ...
      logger.debug(`✅ GÉNÉRATION MIX TERMINÉE - ${itemData.item_code}, URL:`, audioUrl);
      ...
      logger.error(`❌ ERREUR GÉNÉRATION MIX:`, error);
      toast({
  ...
  const handlePlayAudio = (audioUrl: string, title: string) => {
    logger.debug('🔴 === DEBUT handlePlayAudio ===');
    logger.debug('🎵 URL reçue:', audioUrl);
    logger.debug('🎵 generatedAudio actuel:', generatedAudio);
    
    // ✅ Bloquer les URLs de simulation non fonctionnelles
    if (audioUrl.includes('soundjay.com') || audioUrl.includes('fail-buzzer')) {
      logger.debug('⚠️ URL de simulation détectée, pas de lecture possible');
      toast({
    ...
    logger.debug('🎵 BOUTON PLAY CLIQUÉ:', {
    ...
    if (!isValidAudioUrl(audioUrl)) {
      logger.error('❌ URL AUDIO INVALIDE:', audioUrl);
      return;
    }

    logger.debug('🔍 TEST DE CONNECTIVITÉ AUDIO...');
    const testAudio = new Audio();
    
    testAudio.addEventListener('canplay', () => {
      logger.debug('✅ AUDIO PEUT ÊTRE LU, URL VALIDE');
    });
    
    testAudio.addEventListener('error', (e) => {
      logger.error('❌ ERREUR DE TEST AUDIO:', e);
      logger.error('❌ PROBLÈME AVEC L\'URL:', audioUrl);
      // ✅ CORRECTION: Continuer malgré l'erreur de test, le player principal peut fonctionner
      logger.debug('⚠️ Tentative de lecture directe malgré l\'erreur de test...');
    });
    
    testAudio.addEventListener('loadstart', () => {
      logger.debug('🔄 DÉBUT DE CHARGEMENT AUDIO');
    });
    
    testAudio.addEventListener('loadeddata', () => {
      logger.debug('✅ DONNÉES AUDIO CHARGÉES');
    });
    ...
    try {
      ...
      logger.warn('⚠️ Erreur lors du test de l\'URL, mais on continue:', error);
    }

    if (currentTrack?.url === audioUrl && isPlaying) {
      logger.debug('⏸️ PAUSE DE L\'AUDIO EN COURS');
      pause();
    } else {
      // ✅ CORRECTION: Lecture forcée même si test de connectivité échoue
      logger.debug('▶️ LECTURE DU NOUVEL AUDIO (forcée)');
      logger.debug('🎵 Données transmises au contexte audio:', {
        url: audioUrl,
        title: title,
      ...
      try {
        ...
        logger.error('❌ Erreur lors de la lecture audio:', playError);
        toast({
