...
export const isRangBIC5 = (data: any): boolean => {
  ...
  logger.debug('🔍 isRangBIC5 - Theme:', theme);
  logger.debug('📊 isRangBIC5 - Explicit Rang B:', isExplicitRangB);
  
  return isExplicitRangB;
};

export const generateLignesRangAIntelligentIC5 = (data: any): string[][] => {
  logger.debug('🎯 IC-5 Génération Rang A : 4 connaissances selon données');
  
  const lignes: string[][] = [];
  
  // Utiliser les 4 premiers concepts Rang A définis
  conceptsRangAIC5.forEach((concept, index) => {
    logger.debug(`📝 Ajout concept IC-5 Rang A ${index + 1}/4: ${concept.concept.substring(0, 50)}...`);
    const ligne = [
      concept.concept,
      concept.definition,
      concept.exemple,
  ...
  logger.debug(`✅ IC-5 Rang A: ${lignes.length}/4 connaissances générées`);
  return lignes;
};

export const generateLignesRangBIntelligentIC5 = (data: any): string[][] => {
  logger.debug('🎯 IC-5 Génération Rang B : 4 connaissances selon données');
  
  const lignes: string[][] = [];
  
  // Utiliser les 4 concepts Rang B définis
  conceptsRangBIC5.forEach((concept, index) => {
    logger.debug(`📝 Ajout concept IC-5 Rang B ${index + 1}/4: ${concept.concept.substring(0, 50)}...`);
    const ligne = [
      concept.concept,
      concept.definition,
      concept.exemple,
  ...
  logger.debug(`✅ IC-5 Rang B: ${lignes.length}/4 connaissances générées`);
  return lignes;
};

export const determinerColonnesUtilesIC5 = (lignes: string[][]): any[] => {
  logger.debug('🏗️ IC-5: Configuration colonnes optimisée');
...
export function processTableauRangAIC5(data: any) {
  logger.debug('🔍 Processing IC-5 Organisation système de santé:', data);
  
  const isRangB = isRangBIC5(data);
  logger.debug('📊 IC-5 - Est-ce Rang B ?', isRangB);
  ...
  logger.debug('📋 IC-5 - Lignes générées:', lignesEnrichies.length);
  logger.debug('📋 IC-5 - Contenu lignes:', lignesEnrichies);
  ...
  logger.debug(`✅ IC-5 ${isRangB ? 'Rang B' : 'Rang A'}: ${actualCount}/${expectedCount} connaissances`);
