...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès activate-simulation sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour activate-simulation');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative activate-simulation par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ activate-simulation autorisé pour admin ${user.id}`);
