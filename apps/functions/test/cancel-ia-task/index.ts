...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès cancel-ia-task sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour cancel-ia-task');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative cancel-ia-task par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ cancel-ia-task autorisé pour admin ${user.id}`);
