...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès generate-music sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour generate-music');
      return new Response(
    ...
    console.log(`✅ generate-music autorisé pour user ${user.id}`);
