...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès synchronized-lyrics sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour synchronized-lyrics');
      return new Response(
    ...
    console.log(`✅ synchronized-lyrics autorisé pour user ${user.id}`);
