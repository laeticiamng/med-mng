...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès playlist-manager sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour playlist-manager');
      return new Response(
    ...
    console.log(`✅ playlist-manager autorisé pour user ${user.id}`);
