...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès ai-visual-analysis sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour ai-visual-analysis');
      return new Response(
    ...
    console.log(`✅ ai-visual-analysis autorisé pour user ${user.id}`);
