...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès ai-code-analysis sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour ai-code-analysis');
      return new Response(
    ...
    console.log(`✅ ai-code-analysis autorisé pour user ${user.id}`);
