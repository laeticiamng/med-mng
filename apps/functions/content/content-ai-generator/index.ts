...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès content-ai-generator sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour content-ai-generator');
      return new Response(
    ...
    console.log(`✅ content-ai-generator autorisé pour user ${user.id}`);
