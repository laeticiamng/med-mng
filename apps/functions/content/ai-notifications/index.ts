...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès ai-notifications sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour ai-notifications');
      return new Response(
    ...
    console.log(`✅ ai-notifications autorisé pour user ${user.id}`);
