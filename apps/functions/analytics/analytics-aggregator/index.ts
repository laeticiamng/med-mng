...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès analytics-aggregator sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour analytics-aggregator');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative analytics-aggregator par user ${user.id}`);
      return new Response(
