...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès music-generation sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour music-generation');
      return new Response(
    ...
    console.log(`✅ music-generation autorisé pour user ${user.id}`);
