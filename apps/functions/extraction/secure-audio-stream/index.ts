...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès secure-audio-stream sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour secure-audio-stream');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative secure-audio-stream par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ secure-audio-stream autorisé pour admin ${user.id}`);
