...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès extract-edn-uness sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour extract-edn-uness');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative extract-edn-uness par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ extract-edn-uness autorisé pour admin ${user.id}`);
