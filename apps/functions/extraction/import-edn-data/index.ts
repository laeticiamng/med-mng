...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès import-edn-data sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour import-edn-data');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative import-edn-data par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ import-edn-data autorisé pour admin ${user.id}`);
