...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès send-scheduled-reports sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour send-scheduled-reports');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative send-scheduled-reports par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ send-scheduled-reports autorisé pour admin ${user.id}`);
