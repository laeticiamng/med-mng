...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès test-edn-extraction sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour test-edn-extraction');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative test-edn-extraction par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ test-edn-extraction autorisé pour admin ${user.id}`);
