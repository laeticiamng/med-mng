...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès reimport-edn-complete sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour reimport-edn-complete');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative reimport-edn-complete par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ reimport-edn-complete autorisé pour admin ${user.id}`);
