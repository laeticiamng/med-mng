...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès generate-cas-cookie sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour generate-cas-cookie');
      return new Response(
    ...
    console.log(`✅ generate-cas-cookie autorisé pour user ${user.id}`);
