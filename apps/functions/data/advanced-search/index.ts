...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès advanced-search sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour advanced-search');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative advanced-search par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ advanced-search autorisé pour admin ${user.id}`);
