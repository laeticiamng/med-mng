...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès security-metrics sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour security-metrics');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative security-metrics par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ security-metrics autorisé pour admin ${user.id}`);
