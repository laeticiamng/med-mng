...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès secure-streaming-proxy sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour secure-streaming-proxy');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative secure-streaming-proxy par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ secure-streaming-proxy autorisé pour admin ${user.id}`);
