...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès weekly-security-report sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour weekly-security-report');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative weekly-security-report par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ weekly-security-report autorisé pour admin ${user.id}`);
