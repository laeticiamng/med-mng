...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès extraction-monitoring sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour extraction-monitoring');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative extraction-monitoring par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ extraction-monitoring autorisé pour admin ${user.id}`);
