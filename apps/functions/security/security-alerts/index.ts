...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès security-alerts sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour security-alerts');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative security-alerts par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ security-alerts autorisé pour admin ${user.id}`);
