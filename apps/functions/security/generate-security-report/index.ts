...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès generate-security-report sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour generate-security-report');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative generate-security-report par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ generate-security-report autorisé pour admin ${user.id}`);
