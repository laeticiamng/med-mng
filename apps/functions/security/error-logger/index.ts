...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('❌ Tentative accès error-logger sans authentification');
      return new Response(
    ...
    if (authError || !user) {
      console.warn('❌ Token invalide pour error-logger');
      return new Response(
    ...
    if (!isAdmin) {
      console.warn(`❌ Non-admin tentative error-logger par user ${user.id}`);
      return new Response(
    ...
    console.log(`✅ error-logger autorisé pour admin ${user.id}`);
