...
export async function handleRGPD(req: Request, supabase: any, path: string, url: URL): Promise<Response | null> {
  ...
  if (path === '/rgpd/purge' && req.method === 'DELETE') {
    try {
      ...
      // Verify confirmation token (in production, this should be a secure token)
      if (confirmation_token !== `PURGE_${user_id}_CONFIRMED`) {
        return errorResponse(403, 'INVALID_TOKEN', 'Invalid confirmation token');
      }
